...
- **NSE-style option chain** with 11 strikes around ATM, CE/PE LTP, IV, OI per row.
...
- **Multi-leg paper trade execution** — applying a strategy auto-fills every leg at current mock LTPs, stores as one trade.
- **Portfolio** with live unrealized P&L per open trade, square-off button, trade history, win-rate.
- **Gamification** — XP per trade/reward, streak, level, badges (First Trade, Profitable Trade, 7-Day Streak, Pro Member).
- **Pro upgrade** (mock) — flips virtual capital from ₹1L to ₹10L and grants 30 days.

## 4. Tech Stack
- **Frontend**: Expo SDK 54, expo-router (file-based), React Native 0.81, lucide-react-native icons, AsyncStorage, dark mode design system.
- **Backend**: FastAPI + Motor (MongoDB). Black-Scholes-style mock pricing for stable LTPs.
